...
<div class='center wgg-album-footer'>
    <{if $album.nb_images|default:0 > 0}>
        ...
        <{/if}>
        <a class='btn btn-secondary wgg-btn' href='<{$wggallery_url}>/images.php?op=list&amp;alb_id=<{$album.id}>&amp;alb_pid=<{$album.pid}><{if $subm_id|default:false}>&amp;subm_id=<{$subm_id}><{/if}>' title='<{$smarty.const._CO_WGGALLERY_IMAGES_INDEX}>'>
