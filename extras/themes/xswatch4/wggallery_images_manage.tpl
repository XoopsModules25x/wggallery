...
<{if $form|default:false}>
	<{$form}>
<{else}>
    <div class='card panel-<{$panel_type|default:''}>'>
            ...
            <div class='card-body'>
                            ...
                            <li style="display: list-item;" class="mjs-nestedSortable-branch mjs-nestedSortable-collapsed mjs-nestedSortable-leaf" id="menuItem_<{$image.id}>">
                                <div class="menuDiv ui-sortable-handle">
                                    ...
                                    <div class='col-xs-5 wgg-img-sort right'>
                                        <a class='btn btn-secondary wgg-btn-small' href='<{$wggallery_url}>/images.php?op=show&amp;img_id=<{$image.id}>&amp;alb_id=<{$image.albid}>&amp;start=<{$start}>&amp;limit=<{$limit}>&amp;img_submitter=<{$img_submitter|default:0}>&amp;redir_op=manage' title='<{$smarty.const._CO_WGGALLERY_IMAGE_SHOW}>' target='<{$image_target|default:0}>'>
                                        ...
                                        <a class='btn btn-secondary wgg-btn-small' href='images.php?op=rotate&amp;dir=left&amp;img_id=<{$image.id}>&amp;alb_id=<{$alb_id}>&amp;start=<{$start}>&amp;limit=<{$limit}>&amp;img_submitter=<{$img_submitter|default:0}>&amp;redir_op=manage' title='<{$smarty.const._CO_WGGALLERY_IMAGE_ROTATE_LEFT}>'>
                                            <img class='wgg-btn-icon' src='<{$wggallery_icon_url_16}>rotate_left.png' alt='<{$smarty.const._CO_WGGALLERY_IMAGE_ROTATE_LEFT}>'></a>
                                        <a class='btn btn-secondary wgg-btn-small' href='images.php?op=rotate&amp;dir=right&amp;img_id=<{$image.id}>&amp;alb_id=<{$alb_id}>&amp;start=<{$start}>&amp;limit=<{$limit}>&amp;img_submitter=<{$img_submitter|default:0}>&amp;redir_op=manage' title='<{$smarty.const._CO_WGGALLERY_IMAGE_ROTATE_RIGHT}>'>
                ...
                <{else if $showlist|default:false}>
                    <div class=''>
                ...
                <div class='wgg-goback'>
                    <a class='btn btn-secondary wgg-btn' href='<{if $ref}><{$ref}><{else}>images<{/if}>.php?op=<{if $redir_op|default:''}><{$redir_op|default:''}><{else}>list<{/if}>&amp;alb_id=<{$alb_id}>&amp;alb_pid=<{$alb_pid}>' title='<{$smarty.const._CO_WGGALLERY_BACK}>'>
