...
<{/if}>
	...
	<div class="card <{$cols}>">
		...
		<div class="card-body text-center">
            ...
            <{if $showDesc}><p><{$album.desc|default:''}></p><{/if}>
            <p class="center">
                <{if $gallery|default:false}>
                    <a class='btn btn-primary wg-color1' href='<{$wggallery_url}>/gallery.php?op=show&amp;alb_id=<{$album.id}><{if $subm_id|default:false}>&amp;subm_id=<{$subm_id}><{/if}>' title='<{$smarty.const._CO_WGGALLERY_IMAGES_ALBUMSHOW}>' target='<{$gallery_target}>' ><{$smarty.const._CO_WGGALLERY_IMAGES_ALBUMSHOW}></a>
                <{else}>
