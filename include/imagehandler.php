...
class wgImageHandler {
    ...
    public function ResizeAndCrop()
    {
        ...
        if (!imagecopyresampled($thumb, $original, 0,0,0,0, $max_width_resize, $max_height_resize, $original_w, $original_h)) return FALSE;
        ...
        if (!imagecopy($final, $thumb, 0,0, $max_width_offset, $max_height_offset, $max_width_resize, $max_height_resize)) return FALSE;

        // STORE THE FINAL IMAGE - WILL OVERWRITE $this->endFile
        if (!imagejpeg($final, $this->endFile, $this->jpgQuality)) return FALSE;
        return TRUE;
    }
