...
<figure class="effect-<{$hovereffect}> figure<{$number_cols_album}><{$inblock}>">
    ...
    <figcaption>
        <div class="text_figure<{$number_cols_album}><{$inblock}>">
            <h3><{$album.name}></h3>
            <{if $album.desc|default:''}><p><{$album.desc}></p><{/if}>
        </div>
        <{if $album.nb_images|default:''}>
            <{if $gallery|default:''}>
                <a class='' href='<{$wggallery_url}>/gallery.php?op=show&amp;alb_id=<{$album.id}><{if $subm_id|default:''}>&amp;subm_id=<{$subm_id}><{/if}>' title='<{$smarty.const._CO_WGGALLERY_IMAGES_ALBUMSHOW}>' target='<{$gallery_target}>'></a>
            <{else}>
...
<{if $album.linebreak|default:''}></div><div class='clear'>&nbsp;</div><{/if}>