		...
		<div class='panel-body'>
			<{foreach item=album from=$albums_list}>
				<div class='row wgg-album-list'>
					<div class='col-sm-4'>
						<img id='album_<{$album.id}>' class='img-responsive wgg-album-img' src='<{$album.image}>?<{$force}>' alt='<{$smarty.const._CO_WGGALLERY_ALBUM_IMAGE}> <{$album.name}>'>
					</div>
					<div class='col-sm-4'>
					...
					<div class='col-sm-12 center'>
						<{if $album.nb_images|default:''}>
							...
							<{/if}>
							<a class='btn btn-default wgg-btn' href='<{$wggallery_url}>/images.php?op=list&amp;ref=albums&amp;alb_id=<{$album.id}>&amp;alb_pid=<{$album.pid}><{if $subm_id|default:''}>&amp;subm_id=<{$subm_id}><{/if}>' title='<{$smarty.const._CO_WGGALLERY_IMAGES_INDEX}>'>
						...
						<{if $album.nb_subalbums|default:''}>
							<a class='btn btn-default wgg-btn' href='albums.php?op=list&amp;alb_pid=<{$album.id}><{if $subm_id|default:''}>&amp;subm_id=<{$subm_id}><{/if}>' title='<{$smarty.const._CO_WGGALLERY_COLL_ALBUMS}>'>
                ...
                <div class="modal-body">
					<{if $images|default:''}>
						<{foreach item=image from=$images}>
							...
							<input class="img <{if $image.selected|default:''}>wgg-modal-selected<{/if}>" type="image" src="<{$image.medium}>" alt="<{$image.title}>"
